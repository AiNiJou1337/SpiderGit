...
export default function WeeklyTrending() {
  ...
  useEffect(() => {
    async function fetchRepositories() {
      try {
        const res = await fetch('/api/trending?period=weekly')
  ...
  return (
    <div className="container mx-auto py-6">
      {/* 标题区域使用卡片和渐变背景 */}
      <Card className="glass-card bg-gradient-to-br from-purple-500/10 to-violet-500/10 mb-6">
        <CardHeader>
          <CardTitle className="text-3xl font-bold">GitHub 每周趋势</CardTitle>
          <CardDescription className="text-lg">
            展示过去一周内最受欢迎的GitHub项目
          </CardDescription>
        </CardHeader>
      </Card>
